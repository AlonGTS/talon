...
| `mavlink_client.py` | Launches MAVProxy as a subprocess (hardcoded path, see Gotchas below), connects to it via `pymavlink`, and exposes `connect()`, `send_attitude_target()`, `arm_and_set_guided()`, `disarm()`. Runs a background thread that keeps the latest FC `ATTITUDE` cached (needed because `send_attitude_target` composes roll/pitch onto current attitude — see `attitude_changes.md`). |
